...
exports.obtenerTareas = async (req, res) => {
  try {
    ...
    const existeProyecto = await Proyecto.findById(proyecto);
    if (!existeProyecto) {
      return res.status(404).json({ msg: "Proyecto no encontradoo" });
    }
...
exports.actualizarTarea = async (req, res) => {
  try {
    ...
    res.status(500).send("hubo un erroraso ");
  }
};
//eliminar tarea
exports.eliminarTarea = async (req, res) => {
  try {
    ...
    console.log(error);
    res.status(500).send("hubo un errorardo ");
  }
};
